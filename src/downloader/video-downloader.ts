...
// Tải video từ YouTube và Douyin (hỗ trợ cookie VPS)
...
export async function checkDependencies(): Promise<boolean> {
  ...
  // Thông báo trạng thái cookie
  if (config.youtubeCookieFile && fs.existsSync(config.youtubeCookieFile)) {
    logger.success(`Cookie YouTube: ${config.youtubeCookieFile}`);
  } else {
    logger.warn("Không có cookie YouTube - có thể bị chặn trên VPS");
  }
...
export async function downloadOne(url: string): Promise<boolean> {
  ...
  try {
    ...
    if (stats.size / 1024 / 1024 > config.maxVideoSizeMB) {
      logger.warn(`File quá lớn, đang compress...`);
    ...
    await db.videoLibrary.upsert({
      ...
      update: { status: "pending_caption", localPath: filePath, title: info.title, duration: info.duration, fileSize: finalStats.size },
      create: { source, originalUrl: url, localPath: filePath, title: info.title, duration: info.duration, fileSize: finalStats.size, status: "pending_caption" },
    });

    logger.success(`Tải xong: ${info.title.substring(0, 50)} (${(finalStats.size / 1024 / 1024).toFixed(1)}MB)`);
    return true;
  } catch (err: any) {
...
async function saveFailedUrl(url: string) {
  await db.videoLibrary.upsert({
    ...
    create: { source: "unknown", originalUrl: url, localPath: "", status: "failed" },
  });
}
...
export async function bulkDownloadFromFile(filePath: string): Promise<void> {
  ...
  for (let i = 0; i < urls.length; i += concurrency) {
    ...
    if (i + concurrency < urls.length) await new Promise(r => setTimeout(r, 3000));
  }
